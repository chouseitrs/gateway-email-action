...
my $url = "http://$webslingerHost/\@\@\@/";

# Fix attributes in case they dont exist, append to URL
foreach my $attr (@attributes) {
    $ENV{$attr} = "ALL OTHER $attr" if (!defined $ENV{$attr} || $ENV{$attr} eq '');
    $url .= $ENV{$attr} . '/';
}
# The final part of the url is Managed Entity/Dataview.htm 
$url .= $ENV{'_MANAGED_ENTITY'} . '/' . $ENV{'_DATAVIEW'} . '.htm';
